...
function generate_advise_json_subcmd(indent, indent_str, indent_str2, indent_str4,
    ...
    for (i=1; i <= subcmd_len(); ++i) {
        ...
        _ret = _ret "\n" sprintf("printf %s \"\n%s%s: \"; %s printf %s \",\"; ", s_wrap2("%s"), indent_str2 , str_wrap_by_backslash( subcmd_id( i ) ), subcmd_invocation, s_wrap2("%s") )
    }
...
function printf_intermediate_code(arg){
    return sprintf("printf %s %s", s_wrap2("%s"), arg)
}
