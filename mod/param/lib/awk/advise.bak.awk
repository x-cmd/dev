...
function generate_advise_json_subcmd(       i, subcmd_funcname, subcmd_invocation, _name_arr, _ret ){
    for ( i=1; i<=subcmd_len(); ++i ) {
        ...
        subcmd_invocation = sprintf("\\$( PARAM_SUBCMD_DEF=''; X_CMD_ADVISE_FUNC_NAME=%s %s _x_cmd_advise_json %s)", subcmd_funcname, subcmd_funcname, single_quote_string( subcmd_desc(i) ) )
